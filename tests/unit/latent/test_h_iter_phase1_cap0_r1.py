...
from prime_rl.latent import h_iter_phase1_cap0_r1 as contract

ROOT = Path(__file__).parents[3]
RUNNER_PATH = ROOT / "scripts/latent/run_h_iter_phase1_cap0_r1_v1.py"
...
def frozen() -> tuple[dict, dict, dict]:
    ...
    value = load(ROOT / contract.ARTIFACT_DIR / "cap0-r1-contract.json")
...
def plan_fixture() -> dict:
    plan = {
        ...
        "antecedent_evidence_binding": contract.ANTECEDENT_BINDING,
...
def proof_fixture(plan: dict, value: dict, selection: dict) -> dict:
    ...
    proof = {
        ...
        "tamper_audit": {"results": [{"name": name, "rejected": True, "error_type": "CAP0ContractError"} for name in contract.TAMPERS], "rejected_count": 55}, "decision_boundary": contract.DECISION, "proof_sha256": "",
    }
...
def test_contract_materializes_exact(frozen: tuple[dict, dict, dict]) -> None:
    ...
    assert len(contract.TAMPERS) == 55 and len(contract.MEMORY_LABELS) == 28 and len(contract.CACHE_LABELS) == 18
...
def test_r1_antecedent_and_tampers_43_through_55_rejected(frozen: tuple[dict, dict, dict]) -> None:
    value, selection, capture = frozen
    assert runner.validate_antecedent(ROOT) == contract.ANTECEDENT_BINDING
    antecedent = load(ROOT / contract.ANTECEDENT_BINDING["manifest_path"])
    results = runner.run_r1_tampers(value, selection, capture, antecedent)
    assert [row["name"] for row in results] == contract.TAMPERS[42:]
...
def test_mf0_archive_and_train_only_inputs() -> None:
    ...
    contract.validate_contract(load(ROOT / contract.ARTIFACT_DIR / "cap0-r1-contract.json"), selection, capture)
...
def test_atomic_exclusive_terminal(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert writer.write("CAP0-R1-PROOF.json", {"b": 2, "a": 1}) == b'{"a":1,"b":2}\n'
    with pytest.raises(runner.InfrastructureInvalid): writer.write("CAP0-R1-FAILURE.json", {})


def test_launcher_modes_and_bounds() -> None:
    source = (ROOT / "scripts/latent/run_h_iter_phase1_cap0_r1_v1.sh").read_text()
...
def test_model_state_raw_inner_hash_matches_a1nc0_and_scientific_hash_is_unchanged() -> None:
    torch = pytest.importorskip("torch")
    from prime_rl.latent.a1nc0 import tensor_bytes_sha256 as established_tensor_bytes_sha256

    tensors = [
        torch.tensor(1.5, dtype=torch.bfloat16),
        torch.tensor([1.5, -2.0, 0.0], dtype=torch.bfloat16),
        torch.tensor([[1.5, -2.0], [0.0, 4.0]], dtype=torch.bfloat16),
    ]
    for tensor in tensors:
        raw = tensor.detach().cpu().contiguous().reshape(-1).view(torch.uint8).numpy().tobytes()
        header = contract.canonical_json({"dtype": str(tensor.dtype), "shape": list(tensor.shape)}) + b"\n"
        assert runner.tensor_bytes_sha256(torch, tensor) == established_tensor_bytes_sha256(tensor)
        assert runner.tensor_bytes_sha256(torch, tensor) == contract.sha256_bytes(raw)
        assert runner.tensor_sha256(torch, tensor) == contract.sha256_bytes(header + raw)
        assert runner.tensor_sha256(torch, tensor) != runner.tensor_bytes_sha256(torch, tensor)


def test_module_tree_outer_entry_binds_dtype_and_shape() -> None:
    torch = pytest.importorskip("torch")
    ...
    raw = torch.tensor([1, 0], dtype=torch.uint8)
    reshaped = raw.reshape(1, 2)
    typed = raw.view(torch.int16)
    assert runner.tensor_bytes_sha256(torch, raw) == runner.tensor_bytes_sha256(torch, reshaped)
    assert runner.tensor_bytes_sha256(torch, raw) == runner.tensor_bytes_sha256(torch, typed)
    assert len({runner.module_tree_sha256(torch, State(item)) for item in (raw, reshaped, typed)}) == 3
