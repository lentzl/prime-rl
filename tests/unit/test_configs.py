...
def test_sdpo_ttt_smoke_config_repeats_one_attempt_group_per_update():
    ...
    assert config.orchestrator.train.source[0].task_indices == [0]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("02-assignment-rl.toml", "assignment", "standard", 4, 6, 8192, 1),
        ("03-state-rl.toml", "state", "standard", 4, 10, 8192, None),
    ],
)
...
def test_sdpo_accepts_explicit_fused_lm_head():
    config = RLConfig.model_validate(
        {
    ...
    assert config.trainer.model.fused_lm_head_token_chunk_size == 128
