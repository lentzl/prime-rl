...
def wandb_name(branch_name: str) -> str:
    return f"test-reverse-text-rl-sdpo:{branch_name}"
...
def rl_sdpo_process(
    ...
    """Run the RL entrypoint with training_mode = "sdpo"; teacher_inference is
    ...
    cmd = [
        ...
        "rl",
        "@",
        "configs/ci/integration/reverse_text_rl_sdpo/start.toml",
        "--clean-output-dir",
        "--wandb.project",
        wandb_project,
        "--wandb.name",
...
@pytest.fixture(scope="module")
def test_no_error(rl_sdpo_process: ProcessResult, output_dir: Path):
    check_no_error(rl_sdpo_process, output_dir)


def test_eval_avg_goes_up(rl_sdpo_process: ProcessResult, test_no_error, output_dir: Path):
