...
| `trainer.model.optim_cpu_offload` | Full CPU optimizer offload (disabled by default): FP32 masters, moments, and gradients live in CPU RAM, optimizer chunks run on CPU as their final gradients arrive, and persistent BF16 GPU weights refresh during backward. Muon is not supported. |
| `trainer.model.optim_cpu_offload.cpu_optimizer_backend` | CPU AdamW backend. `native` (default) uses a read-only-gradient multi-tensor kernel; `torch` uses fused PyTorch AdamW for debugging and parity checks. |
| `trainer.model.optim_cpu_offload.transfer_buffer_count` | Number of normal-sized pinned BF16 staging slots in each direction. Defaults to `4`; increasing it consumes more pinned RAM and only helps when transfers cannot stay queued. |
| `trainer.model.optim_cpu_offload.max_inflight_backwards` | Per-parameter CUDA-event window for gradient accumulation that outruns CPU transfers. Defaults to `16`. |
| `trainer.model.optim_cpu_offload.timeout_seconds` | Finite timeout for native pipeline waits. Failures report the step, backward generation, pending chunks, and free transfer slots instead of hanging indefinitely. |
| `trainer.model.optim_cpu_offload.numa_bind` | Pin each rank's CPUs to its GPU's NUMA node before state allocation (default `true`). The CPU pipeline is DRAM-bandwidth-bound on multi-socket hosts. |
...
Full CPU optimizer offload moves the optimizer off the GPU entirely (disabled by default):
...
Mutually exclusive with `fsdp_cpu_offload`, and incompatible with `trainer.max_concurrent_runs > 1` (multi-tenant training). Muon is not supported. Gradient offload attaches public post-accumulate hooks to FSDP's sharded parameters, so each finalized gradient starts its CPU transfer immediately after reduce-scatter without accessing private FSDP state.

The native path stores FP32 master weights, moments, and accumulated gradients in pageable CPU slabs. A bounded ring carries BF16 gradients to CPU and refreshed BF16 compute weights back to GPU, so pinned allocation scales with transfer-window and largest-tensor size rather than the whole model shard. During the final backward, each completed gradient chunk runs through the CPU optimizer and immediately starts refreshing its persistent BF16 GPU weights; `optimizer.step()` only drains the remaining pipeline. Hooks only record a preallocated event and enqueue the gradient. Transfer scheduling, event synchronization, conversion, and optimizer work happen on dedicated workers.

Each backward contribution is tagged with its step and backward generation. Duplicate or stale contributions fail explicitly, all host waits have diagnostic timeouts, and checkpoint paths drain both transfer rings before rebuilding GPU weights. The pipeline does not create a Gloo process group or add CPU collectives. Gradient accumulation sums BF16 transfers into FP32 before the update. Set `cpu_optimizer_backend = "torch"` in the offload table to use fused PyTorch AdamW for debugging or numerical comparisons; that fallback prioritizes parity over the bounded native pipeline. The native kernel is compiled and cached for the current CPU capability when the optimizer is created.

Gradient numerics: gradients reduce across ranks in FP32, but FSDP2 materializes them in the BF16 compute model's dtype, so each reduced gradient is rounded to BF16 once before the FP32 CPU update. Disable offloading for gradient numerics bit-faithful to that path.
