#!/usr/bin/env python3
"""Run one bounded full-dense specialist-routing update from protected e33."""
...
from export_q35_2b_specialist_routing_sft_v1 import (
    OBJECTIVE,
    ROUTE_CLASSES,
    ROWS,
    ROWS_PER_ROUTE,
...
SCHEMA_VERSION = "qwen35-2b-specialist-routing-update/v1"
...
def _validated_dataset(path: Path) -> dict[str, Any]:
    ...
    if not manifest_path.is_file() or not parquet.is_file():
        raise FileNotFoundError(f"incomplete specialist routing dataset: {path}")
    manifest = json.loads(manifest_path.read_text())
    expected_counts = {route_class: ROWS_PER_ROUTE for route_class in ROUTE_CLASSES}
    first_batch_counts = manifest.get("first_batch_route_counts")
    if (
        ...
        or manifest.get("training_batch_size") != 16
        or manifest.get("route_counts") != expected_counts
        or not isinstance(first_batch_counts, dict)
        or sorted(first_batch_counts.values()) != [3, 3, 3, 3, 4]
        or set(first_batch_counts) != set(ROUTE_CLASSES)
        or manifest.get("training_template_variants") != [0, 1, 2, 3]
        or manifest.get("heldout_template_variants_excluded") != [4, 5]
        or manifest.get("observed_instance_offset_excluded") != 35100
        or manifest.get("answer_free") is not True
        or manifest.get("public_capability_registry_only") is not True
        or manifest.get("action_and_expert_only_tool_arguments") is not True
        ...
        raise ValueError(f"invalid specialist routing dataset: {path}")
    return manifest
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if source_sha != args.expected_source_sha256:
        raise ValueError(
            "specialist routing parent hash differs from the protected e33 hash"
        )
    ...
    if not complete:
        if not _gpus_idle():
            raise RuntimeError("GPUs are not idle at the routing update boundary")
        env = os.environ.copy()
    ...
    if output_sha == source_sha:
        raise ValueError("specialist routing update did not change the dense weights")

    receipt = {
        "schema_version": SCHEMA_VERSION,
        "status": "complete",
        "algorithm": "sft_public_registry_specialist_routing_v1",
        "role": "coordinator",
        "optimizer_updates": args.optimizer_updates,
        "full_dense": True,
        ...
        "dataset": {
            ...
            "rows": dataset["rows"],
            "route_counts": dataset["route_counts"],
            "first_batch_route_counts": dataset["first_batch_route_counts"],
            "answer_free": True,
        },
        "training": {
...
def main() -> None:
    ...
    if args.optimizer_updates not in {1, 2}:
        parser.error("routing update count must be one or two")
    if args.batch_size != 16:
        parser.error("routing batch size must remain 16")
