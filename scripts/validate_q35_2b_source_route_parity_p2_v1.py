#!/usr/bin/env python3
"""Prospective fail-closed P2 LR=0 route and mechanism admission."""
...
CONFIG = Path("experiments/qwen35-2b-document-recursion-zero-update-v1/specialist-source-route-parity-p2-lr0.toml")
OUTPUT_ROOT = Path("/home/ubuntu/rlm/outputs/q35-2b-source-route-parity-p2-v1")
RUN_DIR = OUTPUT_ROOT / "lr0-calibration" / "source-route-parity-p2-lr0-admission"
RESULT_ROOT = Path("/home/ubuntu/rlm/results/q35-2b-source-route-parity-p2-v1")
...
SAMPLING_CONTRACT = Path("experiments/qwen35-2b-document-recursion-zero-update-v1/specialist-source-route-parity-p2-sampling-v1.json")
SEED = 20270925
OFFSET = 72000
MODEL_HASHES = {
...
def write_preflight_model_hashes(path: Path) -> dict[str, object]:
    report = {
        "schema_version": "q35-2b-source-route-parity-p2-model-preflight/v1",
...
def write_postflight_model_hashes(
    ...
    *,
    ...
    report = {
        "schema_version": "q35-2b-source-route-parity-p2-model-postflight/v1",
...
def validate_resolved_config_only(run_dir: Path) -> dict[str, Any]:
    ...
    return {
        "schema_version": "q35-2b-source-route-parity-p2-resolved-preflight/v1",
        "run_dir": str(run_dir),
...
def validate_runtime(
    ...
    *,
    ...
    if (
        ...
        != "q35-2b-source-route-parity-p2-model-postflight/v1"
    ...
    return {
        "schema_version": "q35-2b-source-route-parity-p2-mechanism-admission/v1",
...
def main() -> None:
    ...
    try:
        if args.validate_resolved_configs:
            ...
            if args.preflight_model_hashes is None or args.postflight_model_hashes is None:
                raise BASE.AuditFailure("P2 runtime requires model preflight and postflight evidence")
