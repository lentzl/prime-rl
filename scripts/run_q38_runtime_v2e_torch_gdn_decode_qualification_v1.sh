...
output_root=${RUNTIME_V2E_OUTPUT_ROOT:-/home/ubuntu/rlm/results/q38-runtime-v2e-torch-gdn-decode-qualification-v1}
...
soak_draws=5
quiescence_seconds=${RUNTIME_V2E_QUIESCENCE_SECONDS:-90}
wrapper_path=$root/scripts/run_q38_runtime_v2e_torch_gdn_decode_qualification_v1.sh
wrapper_sha256=$(sha256sum "$wrapper_path" | awk '{print $1}')
patch_path=$root/experiments/qwen38-to-qwen35-2b-role-distillation-v1/runtime-v2e-torch-gdn-decode.patch
patch_sha256=4b0fc4a086bb81c6bd3cfc239e422fc0fd898f7e949bb5bb94bdf09bf36d8b23
site_packages=$runtime_env/lib/python3.12/site-packages
qwen_module_rel=vllm/model_executor/layers/mamba/gdn/qwen_gdn_linear_attn.py
qwen_module=$site_packages/$qwen_module_rel
qwen_original_sha256=1227d6f385a52296e9f08223544b1c5fdc7e8d9aa09a848e7a8e522a8dc51214
qwen_patched_sha256=28096a290180177fe5af07a9a040ae6a39af4814ff8a7390a0b44a806215cf23

if [[ ! -f "$patch_path" ]] || [[ "$(sha256sum "$patch_path" | awk '{print $1}')" != "$patch_sha256" ]]; then
  echo "runtime-v2e patch is missing or has an unexpected hash" >&2
  exit 1
fi
if [[ ! -f "$qwen_module" ]] || [[ "$(sha256sum "$qwen_module" | awk '{print $1}')" != "$qwen_original_sha256" ]]; then
  echo "runtime-v2e refuses an unexpected or already patched Qwen GDN module" >&2
  exit 1
fi

if [[ ! "$quiescence_seconds" =~ ^[1-9][0-9]*$ ]]; then
  echo "runtime-v2e quiescence must be a positive integer" >&2
  exit 1
fi
...
if [[ -n "$(nvidia-smi --query-compute-apps=pid --format=csv,noheader)" ]]; then
  echo "refusing runtime-v2e qualification while a GPU process is active" >&2
  exit 1
fi
if [[ -e "$output_root" ]]; then
  echo "refusing to append to or overwrite runtime-v2e output: $output_root" >&2
  exit 1
fi
mkdir -p "$output_root"

qwen_backup=$output_root/QWEN-GDN-ORIGINAL.py
runtime_patched=false
restore_runtime() {
  if [[ "$runtime_patched" == true ]]; then
    cp --preserve=mode,timestamps "$qwen_backup" "$qwen_module"
    runtime_patched=false
    if [[ "$(sha256sum "$qwen_module" | awk '{print $1}')" != "$qwen_original_sha256" ]]; then
      echo "runtime-v2e failed to restore the original Qwen GDN module" >&2
      return 1
    fi
  fi
}
trap restore_runtime EXIT
cp --preserve=mode,timestamps "$qwen_module" "$qwen_backup"
patch --batch --forward -d "$site_packages" -p1 <"$patch_path"
runtime_patched=true
if [[ "$(sha256sum "$qwen_module" | awk '{print $1}')" != "$qwen_patched_sha256" ]]; then
  echo "runtime-v2e patched Qwen GDN module hash mismatch" >&2
  exit 1
fi
"$runtime_env/bin/python" -m py_compile "$qwen_module"
"$runtime_env/bin/python" - <<'PY'
import inspect

from vllm.model_executor.layers.mamba.gdn import qwen_gdn_linear_attn

source = inspect.getsource(qwen_gdn_linear_attn._torch_sigmoid_gating_delta_rule_update)
assert "torch.einsum" in source
PY

require_gpu_quiescence() {
  local second
  for second in $(seq 1 "$quiescence_seconds"); do
    if [[ -n "$(nvidia-smi --query-compute-apps=pid --format=csv,noheader)" ]]; then
      echo "GPU process appeared during runtime-v2e quiescence barrier at second $second" >&2
      return 1
    fi
    sleep 1
  done
...
write_terminal_manifest() {
  ...
  local terminal_manifest=$output_root/RUNTIME-V2E-${status^^}.txt
  {
    printf 'schema=q38-runtime-v2e-torch-gdn-decode-terminal/v1\n'
    printf 'status=%s\n' "$status"
    printf 'failed_leg=%s\n' "$failed_leg"
    printf 'runtime_lane=runtime-v2e-torch-gdn-decode\n'
    ...
    printf 'soak_draws=%s\n' "$soak_draws"
    printf 'interleg_gpu_quiescence_seconds=%s\n' "$quiescence_seconds"
    printf 'torch_gdn_decode_patch_sha256=%s\n' "$patch_sha256"
    printf 'qwen_gdn_original_sha256=%s\n' "$qwen_original_sha256"
    printf 'qwen_gdn_patched_sha256=%s\n' "$qwen_patched_sha256"
    sha256sum "$qwen_backup" "$patch_path"
    printf 'finished_at_utc=%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
    printf 'wrapper_sha256=%s\n' "$wrapper_sha256"
    find "$output_root" -mindepth 2 -maxdepth 2 \
      -name RUNTIME-V2E-SHA256SUMS.txt -print0 \
      | sort -z \
      | xargs -0 -r sha256sum
  } >"$terminal_manifest"
...
run_leg() {
  ...
  local start_index=$3
  local draws=$4
  local label=q38-runtime-v2e-${phase}-3806011-r${attempt}
  ...
  if ! require_gpu_quiescence; then
    echo "runtime-v2e quiescence barrier failed before leg $label" >&2
    return 1
  fi
  if [[ -e "$run_dir" ]]; then
    echo "refusing to overwrite runtime-v2e leg: $run_dir" >&2
    return 1
  fi
  ...
  if [[ ! -d "$run_dir" ]]; then
    echo "runtime-v2e leg produced no run directory: $label" >&2
    return 1
  fi
...
PY
  ...
  if grep -Eq 'fused_recurrent_gated_delta_rule_packed_decode_kernel|fused_sigmoid_gating_delta_rule_update_kernel' \
    "$run_dir/inference.log"; then
    echo "a forbidden Triton GDN decode kernel appeared despite runtime-v2e patch" \
      >"$run_dir/INTERVENTION-VALIDATION.stderr"
    intervention_status=1
  else
    printf '%s\n' \
      'torch GDN decode patch active; packed and fused-sigmoid decode JIT signatures absent' \
  ...
  {
    printf 'schema=q38-runtime-v2e-torch-gdn-decode-leg/v1\n'
    printf 'runtime_lane=runtime-v2e-torch-gdn-decode\n'
    ...
    printf 'cuda_launch_blocking=1\n'
    printf 'vllm_enable_fla_packed_recurrent_decode=0\n'
    printf 'prestart_gpu_quiescence_seconds=%s\n' "$quiescence_seconds"
    printf 'torch_gdn_decode_patch_sha256=%s\n' "$patch_sha256"
    printf 'qwen_gdn_patched_sha256=%s\n' "$qwen_patched_sha256"
    ...
    printf 'wrapper_sha256=%s\n' "$wrapper_sha256"
  } >"$run_dir/RUNTIME-V2E-LEG.txt"
  find "$run_dir" -type f ! -name RUNTIME-V2E-SHA256SUMS.txt -print0 \
    | sort -z \
    | xargs -0 sha256sum \
    >"$run_dir/RUNTIME-V2E-SHA256SUMS.txt"

  if [[ $run_status -ne 0 || $audit_status -ne 0 \
    || $forbidden_status -ne 0 || $intervention_status -ne 0 \
    || $teardown_status -ne 0 ]]; then
    echo "runtime-v2e leg $label failed infrastructure qualification" >&2
    return 1
  fi
}
...
echo "runtime-v2e torch GDN decode qualification passed"
