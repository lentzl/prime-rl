...
output_root=${RUNTIME_V2D_OUTPUT_ROOT:-/home/ubuntu/rlm/results/q38-runtime-v2d-quiescent-no-packed-decode-qualification-v1}
...
soak_draws=5
quiescence_seconds=${RUNTIME_V2D_QUIESCENCE_SECONDS:-90}
wrapper_path=$root/scripts/run_q38_runtime_v2d_quiescent_no_packed_decode_qualification_v1.sh
wrapper_sha256=$(sha256sum "$wrapper_path" | awk '{print $1}')

if [[ ! "$quiescence_seconds" =~ ^[1-9][0-9]*$ ]]; then
  echo "runtime-v2d quiescence must be a positive integer" >&2
  exit 1
fi
...
if [[ -n "$(nvidia-smi --query-compute-apps=pid --format=csv,noheader)" ]]; then
  echo "refusing runtime-v2d qualification while a GPU process is active" >&2
  exit 1
fi
if [[ -e "$output_root" ]]; then
  echo "refusing to append to or overwrite runtime-v2d output: $output_root" >&2
  exit 1
fi
mkdir -p "$output_root"
...
require_gpu_quiescence() {
  ...
  for second in $(seq 1 "$quiescence_seconds"); do
    if [[ -n "$(nvidia-smi --query-compute-apps=pid --format=csv,noheader)" ]]; then
      echo "GPU process appeared during runtime-v2d quiescence barrier at second $second" >&2
      return 1
    fi
    sleep 1
  done
...
write_terminal_manifest() {
  ...
  local terminal_manifest=$output_root/RUNTIME-V2D-${status^^}.txt
  {
    printf 'schema=q38-runtime-v2d-quiescent-no-packed-decode-terminal/v1\n'
    printf 'status=%s\n' "$status"
    printf 'failed_leg=%s\n' "$failed_leg"
    printf 'runtime_lane=runtime-v2d-quiescent-no-packed-decode\n'
    ...
    find "$output_root" -mindepth 2 -maxdepth 2 \
      -name RUNTIME-V2D-SHA256SUMS.txt -print0 \
      | sort -z \
      | xargs -0 -r sha256sum
  } >"$terminal_manifest"
...
run_leg() {
  ...
  local start_index=$3
  local draws=$4
  local label=q38-runtime-v2d-${phase}-3806011-r${attempt}
  ...
  if ! require_gpu_quiescence; then
    echo "runtime-v2d quiescence barrier failed before leg $label" >&2
    return 1
  fi
  if [[ -e "$run_dir" ]]; then
    echo "refusing to overwrite runtime-v2d leg: $run_dir" >&2
    return 1
  fi
  ...
  if [[ ! -d "$run_dir" ]]; then
    echo "runtime-v2d leg produced no run directory: $label" >&2
    return 1
  fi
...
PY
  ...
  if grep -q 'fused_recurrent_gated_delta_rule_packed_decode_kernel' \
    ...
    echo "packed recurrent decode kernel appeared despite runtime-v2d flag" \
  ...
  {
    printf 'schema=q38-runtime-v2d-quiescent-no-packed-decode-leg/v1\n'
    printf 'runtime_lane=runtime-v2d-quiescent-no-packed-decode\n'
    ...
    printf 'wrapper_sha256=%s\n' "$wrapper_sha256"
  } >"$run_dir/RUNTIME-V2D-LEG.txt"
  find "$run_dir" -type f ! -name RUNTIME-V2D-SHA256SUMS.txt -print0 \
    | sort -z \
    | xargs -0 sha256sum \
    >"$run_dir/RUNTIME-V2D-SHA256SUMS.txt"

  if [[ $run_status -ne 0 || $audit_status -ne 0 \
    || $forbidden_status -ne 0 || $intervention_status -ne 0 \
    || $teardown_status -ne 0 ]]; then
    echo "runtime-v2d leg $label failed infrastructure qualification" >&2
    return 1
  fi
}
...
echo "runtime-v2d quiescent no-packed-decode qualification passed"
