...
config=${DOCUMENT_TRANSLATION_CONFIG:-$root/experiments/qwen35-2b-document-translation-prime-agent-v1/smoke.toml}
output_root=${QWEN38_QUALIFICATION_OUTPUT_ROOT:-/home/ubuntu/rlm/results/q35-2b-document-translation-prime-agent-v1}
...
if [[ -z "$uv_bin" || ! -x "$eval_bin" || ! -x "$runtime_python" ]]; then
  echo "document translation evaluation runtime is incomplete" >&2
  exit 1
fi
if [[ ! -f "$config" ]]; then
  echo "document translation config not found: $config" >&2
  exit 1
fi

cd "$root"
export PYTHONPATH="$root/deps/verifiers/environments/document_translation_v1:$root/deps/verifiers${PYTHONPATH:+:$PYTHONPATH}"
mkdir -p "$run_output"
{
...
"$eval_bin" @ "$config" \
  ...
  --run.name document-translation \
  --run.dir document-translation

"$uv_bin" run --no-sync scripts/summarize_prime_agent_mastery_v2.py \
  "$run_output/document-translation" \
  --expected-count 1 \
