...
config_sha=${3:?root-frozen P1 config SHA-256 required}
task_bank_sha=${4:?root-frozen P1 task-bank SHA-256 required}
task_key_set_sha=${5:?root-frozen P1 task-key-set SHA-256 required}
sampling_sha=${6:?root-frozen P1 sampling-contract SHA-256 required}
dry_run=${P1_DRY_RUN:-false}

config=experiments/qwen35-2b-document-recursion-zero-update-v1/\
specialist-source-route-parity-p1-lr0.toml
validator=scripts/validate_q35_2b_source_route_parity_p1_v1.py
sampling_contract=experiments/qwen35-2b-document-recursion-zero-update-v1/\
specialist-source-route-parity-p1-sampling-v1.json
...
output_root=/home/ubuntu/rlm/outputs/q35-2b-source-route-parity-p1-v1
run_dir=$output_root/lr0-calibration/source-route-parity-p1-lr0-admission
result_root=/home/ubuntu/rlm/results/q35-2b-source-route-parity-p1-v1
...
p1_pythonpath="$root/src:$root/packages/prime-rl-configs/src:$root/deps/verifiers/environments/subagent_communication_v1:$root/deps/verifiers/environments/source_worker_first_call_v1"
export PYTHONPATH="$p1_pythonpath${PYTHONPATH:+:$PYTHONPATH}"

cd "$root"
if [[ "$(git rev-parse HEAD)" != "$revision" ]]; then
  echo "P1 execution revision mismatch" >&2
  exit 1
fi
if [[ -n "$(git status --porcelain)" ]]; then
  echo "P1 requires a clean parent worktree" >&2
  exit 1
fi
if [[ "$(git -C deps/verifiers rev-parse --show-toplevel 2>/dev/null)" != "$root/deps/verifiers" ]]; then
  echo "P1 requires an initialized Verifiers submodule" >&2
  exit 1
fi
actual_verifiers_revision=$(git -C deps/verifiers rev-parse HEAD)
if [[ "$actual_verifiers_revision" != "$verifiers_revision" ]]; then
  echo "P1 Verifiers revision mismatch" >&2
  exit 1
fi
if [[ "$actual_verifiers_revision" == "$legacy_verifiers_gitlink" || "$actual_verifiers_revision" == "$live_detached_verifiers_prefix"* ]]; then
  echo "P1 refuses legacy or detached-live Verifiers revisions" >&2
  exit 1
fi
if [[ -n "$(git -C deps/verifiers status --porcelain)" ]]; then
  echo "P1 requires a clean durable Verifiers checkout" >&2
  exit 1
fi
for package in subagent_communication_v1 source_worker_first_call_v1; do
  if [[ ! -f "$root/deps/verifiers/environments/$package/pyproject.toml" ]]; then
    echo "P1 Verifiers package is missing: $package" >&2
    exit 1
  fi
done
if [[ ! "$revision" =~ ^[0-9a-f]{40}$ \
  ...
  echo "P1 requires exact revision and config identities" >&2
  exit 1
fi
...
if [[ -e "$output_root" || -e "$result_root" ]]; then
  echo "refusing duplicate or partial P1 output/result root" >&2
  exit 1
fi
...
run_p1_python "$validator" "$config"
run_p1_python scripts/hash_q35_2b_source_route_parity_p1_tasks_v1.py "$config" \
  --expected-task-bank-sha256 "$task_bank_sha" \
  --expected-task-key-set-sha256 "$task_key_set_sha"

if [[ "$dry_run" == true ]]; then
  run_p1_rl "$config" --dry-run
  echo "P1 static and launcher dry-run checks passed"
  exit 0
fi

mapfile -t gpu_names < <(nvidia-smi --query-gpu=name --format=csv,noheader)
if [[ ${#gpu_names[@]} -ne 2 ]] || printf '%s\n' "${gpu_names[@]}" | grep -qv 'RTX A6000'; then
  echo "P1 requires exactly 2x RTX A6000" >&2
  exit 1
fi
if [[ -n "$(nvidia-smi --query-compute-apps=pid --format=csv,noheader)" ]]; then
  echo "refusing P1 launch while another GPU process is active" >&2
  exit 1
fi
...
if [[ ! -f "$run_dir/metrics.jsonl" \
  ...
  echo "P1 did not produce one complete unrecorded calibration" >&2
  exit 1
fi
if find "$run_dir/checkpoints" "$run_dir/weights" -type f -print -quit 2>/dev/null | grep -q .; then
  echo "P1 wrote a forbidden model artifact" >&2
  exit 1
fi
rehash_protected_models
run_p1_python "$validator" "$run_dir" --runtime \
...
echo "P1 mechanism admission recorded; no checkpoint or nonzero optimizer authorization: $admission_result"
