...
config=${NATURAL_YIELD_SDPO_UPDATE_CONFIG:-$root/experiments/qwen35-27b-procedural-harness-master-v1/natural-yield-sdpo-update.toml}
run_dir=/ephemeral/outputs/qwen35-27b-natural-yield-sdpo-v1/low-dose-r1
...
validator_module=scripts.validate_natural_yield_sdpo_update_v1
postflight=$root/scripts/run_qwen35_27b_natural_yield_sdpo_postflight_v1.sh
...
for path in "$config" "$prerequisite" "$postflight"; do
  if [[ ! -f "$path" ]]; then
    echo "required natural-yield update input is absent: $path" >&2
    exit 1
  fi
done
if [[ -e "$run_dir" ]]; then
  echo "refusing to overwrite natural-yield update: $run_dir" >&2
  exit 1
fi
...
if [[ "${NATURAL_YIELD_SDPO_UPDATE_DRY_RUN:-false}" == true ]]; then
  dry_run_root=$(mktemp -d /tmp/natural-yield-sdpo-update-dry-run.XXXXXX)
  ...
  rl @ "$config" \
    ...
    --run.dir preflight \
    --dry-run
  echo "natural-yield low-dose SDPO preflight passed"
  exit 0
fi
...
"$postflight" "$model_snapshot" "$weights"
