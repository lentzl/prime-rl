#!/usr/bin/env python3
"""Prospective fail-closed P1 LR=0 route and mechanism admission."""
...
SPEC = importlib.util.spec_from_file_location("p1_base_validator", BASE_PATH)
...
CONFIG = Path("experiments/qwen35-2b-document-recursion-zero-update-v1/specialist-source-route-parity-p1-lr0.toml")
OUTPUT_ROOT = Path("/home/ubuntu/rlm/outputs/q35-2b-source-route-parity-p1-v1")
RUN_DIR = OUTPUT_ROOT / "lr0-calibration" / "source-route-parity-p1-lr0-admission"
RESULT_ROOT = Path("/home/ubuntu/rlm/results/q35-2b-source-route-parity-p1-v1")
...
SAMPLING_CONTRACT = Path("experiments/qwen35-2b-document-recursion-zero-update-v1/specialist-source-route-parity-p1-sampling-v1.json")
SEED = 20270917
OFFSET = 71000
MODEL_HASHES = {
...
def write_preflight_model_hashes(path: Path) -> dict[str, object]:
    report = {
        "schema_version": "q35-2b-source-route-parity-p1-model-preflight/v1",
...
def validate_config(path: Path) -> dict[str, Any]:
    ...
    if path != CONFIG:
        raise BASE.AuditFailure("P1 requires its exact prospective config path")
    if (
        ...
        raise BASE.AuditFailure("P1 write-once identity or no-eval contract changed")
    return {**report, "seed": SEED, "offset": OFFSET, "mechanism_admission_only": True}


def _validate_p1_health(
    ...
    if any(trace.get("ok") is not True or trace.get("errors") for trace in traces):
        raise BASE.AuditFailure("P1 requires all traces ok with no trace errors")
    ...
    for trace in traces:
        ...
        if family not in BASE.EXPECTED_FAMILIES or not isinstance(key, str) or not isinstance(reward, (int, float)):
            raise BASE.AuditFailure("P1 trace lacks family/key/reward identity")
        keys.add(key)
        stops[family][trace.get("stop_condition")] += 1
        rewards[family].append(float(reward))
    p1_bank_keys = set(bank["task_keys"])
    p0_bank_keys = set(bank["p0_task_keys"])
    if not keys <= p1_bank_keys or keys & p0_bank_keys or len(keys) != 2:
        raise BASE.AuditFailure("P1 selected keys are not a disjoint subset of its frozen bank")
    for family in BASE.EXPECTED_FAMILIES:
        if stops[family]["agent_completed"] < 1 or stops[family]["max_turns"] > 7:
            raise BASE.AuditFailure(f"P1 termination gate failed for {family}")
        ...
        if (
            ...
            raise BASE.AuditFailure(f"P1 reward-contrast gate failed for {family}")
    ...
    if (
        ...
        raise BASE.AuditFailure("P1 aggregate likelihood-parity gate failed")
    ...
    for index, export in enumerate(exports):
        ...
        if source not in BASE.EXPECTED_SOURCE_FAMILIES or not isinstance(mask, list) or not isinstance(masked, list) or not isinstance(mismatch, list) or not isinstance(entropy, list) or len(mask) != len(masked) or len(mask) != len(mismatch) or len(mask) != len(entropy) or (weights is not None and (not isinstance(weights, list) or len(weights) != len(mask))):
            raise BASE.AuditFailure(f"P1 export {index} lacks source/mask evidence")
        active_indices = [
            position
        ...
        if not active_indices:
            raise BASE.AuditFailure("P1 export has no active trainable tokens")
        masked_fraction = sum(bool(masked[position]) for position in active_indices) / len(active_indices)
        if masked_fraction > 0.02:
            raise BASE.AuditFailure("P1 per-export mask fraction exceeds 0.02")
        ...
        for position in active_indices:
            ...
            if not isinstance(mismatch_value, (int, float)) or not isinstance(entropy_value, (int, float)) or not math.isfinite(mismatch_value) or not math.isfinite(entropy_value):
                raise BASE.AuditFailure("P1 export lacks finite likelihood evidence")
    ...
    if (
        ...
        raise BASE.AuditFailure("P1 export token/masking gate failed")
    for source in BASE.EXPECTED_SOURCE_FAMILIES:
        weighted = masked_active_by_family[source] / active_by_family[source]
        if weighted > 0.01:
            raise BASE.AuditFailure(f"P1 family export masking exceeds 0.01 for {source}")
        ...
        if (
            ...
            raise BASE.AuditFailure(f"P1 export-derived family mismatch gate failed for {source}")
    export_likelihood = {
        source: {
    ...
    if (
        ...
        raise BASE.AuditFailure("P1 export-derived aggregate mismatch gate failed")
    return {
...
def validate_runtime(
    ...
    *,
    ...
    if (
        ...
        raise BASE.AuditFailure("P1 runtime/config/task-bank identity changed")
    preflight = BASE._read_json(preflight_model_hashes)
    if (
        preflight.get("schema_version")
        != "q35-2b-source-route-parity-p1-model-preflight/v1"
        or preflight.get("models") != _actual_model_hashes()
    ):
        raise BASE.AuditFailure("P1 protected-model preflight/postflight evidence differs")
    postflight_models = _actual_model_hashes()
    structural = BASE.validate_runtime(
        run_dir,
        "audit",
        routing_audit_path=ROUTE_AUDIT,
        p1_timed_route_admission=True,
    ...
    hash_path = HERE / "hash_q35_2b_source_route_parity_p1_tasks_v1.py"
    hash_spec = importlib.util.spec_from_file_location("p1_task_hasher", hash_path)
    if hash_spec is None or hash_spec.loader is None:
        raise BASE.AuditFailure("cannot load P1 task-bank materializer")
    hasher = importlib.util.module_from_spec(hash_spec)
    hash_spec.loader.exec_module(hasher)
    bank = hasher.materialize(CONFIG)
    if (
        bank["task_bank_sha256"] != task_bank_sha256
        or bank["task_key_set_sha256"] != task_key_set_sha256
        or bank["tasks"] != 64
        or bank["p0_tasks"] != 64
        or bank["p0_disjoint"] is not True
    ):
        raise BASE.AuditFailure("P1/P0 full task-bank freeze or disjointness changed")
    observed = BASE._observe_lr0_health(metrics, traces)["metrics"]
    health = _validate_p1_health(structural, traces, exports, bank, observed)
    return {
        "schema_version": "q35-2b-source-route-parity-p1-mechanism-admission/v1",
        ...
        "p1_authorizes_nonzero_update": False,
...
def main() -> None:
    ...
    try:
        if args.write_preflight_model_hashes is not None:
            validate_config(args.target)
            report = write_preflight_model_hashes(args.write_preflight_model_hashes)
        elif args.runtime:
            if args.preflight_model_hashes is None:
                raise BASE.AuditFailure("P1 runtime requires model preflight evidence")
        ...
        raise SystemExit(f"P1 validation failed: {error}") from error
