...
if [[ "${A0_OWNER_APPROVED:-}" != "1" ]]; then
  echo "A0_OWNER_APPROVED=1 is required after root reviews the frozen plan" >&2
  exit 2
fi

repo=${1:?absolute repository path required}
run_id=${2:?exclusive A0 run ID required}
plan="$repo/experiments/qwen35-2b-latent-workspace-v1/a0-mechanism-plan-v1.json"
...
if [[ ! "$run_id" =~ ^a0-[a-z0-9][a-z0-9._-]{2,63}$ ]]; then
  echo "run ID must be a0- followed by 3-64 lowercase namespace characters" >&2
  exit 2
fi
if [[ -L "$plan" || -L "$bank" || ! -f "$plan" || ! -f "$bank" ]]; then
  echo "frozen A0 plan or bank is absent or symlinked" >&2
  exit 2
fi
...
if [[ ! "$execution_commit" =~ ^[0-9a-f]{40}$ ]]; then
  echo "A0 execution commit is not an exact lowercase 40-character hash" >&2
  exit 2
fi
if [[ -n "$(git status --porcelain --untracked-files=all)" ]]; then
  echo "A0 refuses to launch from a dirty worktree" >&2
  exit 2
fi
...
if [[ "$output_entries" != "receipt.json" || "$output_bytes" -gt 16777216 ]]; then
  echo "A0 output namespace contains unexpected files or exceeds 16 MiB" >&2
  exit 2
fi
