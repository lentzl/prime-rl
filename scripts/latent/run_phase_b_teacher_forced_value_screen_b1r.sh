...
readonly BANK_DIR="$EXPERIMENT_DIR/phase-b-b1-bank-v1"
readonly OUTPUT_DIR=/home/ubuntu/rlm/results/q35-2b-phase-b1r-teacher-forced-value-screen-v1
...
[[ $(sha256sum "$EXPERIMENT_DIR/phase-b-teacher-forced-value-screen-b1r-plan.json" | cut -d' ' -f1) == "$ROOT_AUTHORIZED_PLAN_SHA256" ]]
(cd "$EXPERIMENT_DIR" && sha256sum --check phase-b-teacher-forced-value-screen-b1r.sha256)
...
# The Python bound reserves five minutes for failure auditing and one minute
# for exclusive terminal receipt publication inside this four-hour outer cap.
exec timeout --signal=TERM --kill-after=60s 4h \
  "$UV" run --project "$UV_PROJECT" --no-sync python \
  scripts/latent/run_phase_b_teacher_forced_value_screen_v1.py \
  --plan "$EXPERIMENT_DIR/phase-b-teacher-forced-value-screen-b1r-plan.json" \
  --train-selection "$BANK_DIR/training-selection.json" \
  --heldout-selection "$BANK_DIR/heldout-selection.json" \
