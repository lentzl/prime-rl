...
from prime_rl.latent.h_iter_phase1_cap0_r1 import (
    ANTECEDENT_BINDING,
    ARTIFACT_DIR,
    CACHE_LABELS,
    COUNTS,
...
PLAN_RELATIVE_PATH = f"{ARTIFACT_DIR}/cap0-r1-plan.json"
CONTRACT_RELATIVE_PATH = f"{ARTIFACT_DIR}/cap0-r1-contract.json"
...
PLAN_ASSET_PATHS = [
    ...
    ANTECEDENT_BINDING["manifest_path"],
    f"{ARTIFACT_DIR}/cap0-preflight-run1.preflight.json",
    f"{ARTIFACT_DIR}/cap0-preflight-run1.stderr.log",
    f"{ARTIFACT_DIR}/cap0-preflight-run1.exit.txt",
    f"{ARTIFACT_DIR}/cap0-run1.CAP0-FAILURE.json",
    f"{ARTIFACT_DIR}/cap0-run1.launcher.log",
    f"{ARTIFACT_DIR}/cap0-run1.exit.txt",
    "pyproject.toml",
    "uv.lock",
    "src/prime_rl/latent/h_iter_phase1_cap0_r1.py",
    "scripts/latent/run_h_iter_phase1_cap0_r1_v1.py",
    "scripts/latent/run_h_iter_phase1_cap0_r1_v1.sh",
    "scripts/latent/freeze_h_iter_phase1_cap0_r1_plan_v1.py",
    "tests/unit/latent/test_h_iter_phase1_cap0_r1.py",
]
EXPERIMENT_READ_PATHS = [path for path in PLAN_ASSET_PATHS if path.startswith("experiments/")]
...
TERMINAL_CONTRACT = {"success_file": "CAP0-R1-PROOF.json", "failure_file": "CAP0-R1-FAILURE.json", "exclusive_atomic": True, "canonical_roundtrip_twice": True, "success_status": PROOF_STATUS, "failure_statuses": [REJECT_STATUS, INCOMPLETE_STATUS, EXPOSURE_STATUS, "infrastructure_invalid"]}
...
def validate_plan(plan: dict[str, Any], external_sha256: str | None = None) -> None:
    keys = {"schema_version", "status", "mechanism", "run_identity", "mechanism_code_commit", "execution_authorization", "output_root", "remote_paths", "runtime", "asset_sha256", "mf0_archive_binding", "antecedent_evidence_binding", "probe_contract", "model_contract", "cache_contract", "resource_bounds", "memory_label_schedule", "terminal_contract", "safety_boundary", "full_freeze", "plan_sha256"}
    ...
    expected = {
        ...
        "antecedent_evidence_binding": ANTECEDENT_BINDING,
...
def validate_antecedent_payload(manifest: dict[str, Any]) -> None:
    top_keys = {"schema_version", "status", "mechanism", "run_identity", "ordered_evidence_assets", "prior_preflight", "antecedent_failure", "repair_boundary", "manifest_sha256"}
    if set(manifest) != top_keys or manifest["manifest_sha256"] != ANTECEDENT_BINDING["manifest_internal_sha256"]:
        raise CAP0ContractError("CAP0-R1 antecedent manifest differs")
    if manifest["schema_version"] != "prime-rl/latent-h-iter-phase1-cap0-r1-antecedent-evidence-manifest/v1" or manifest["status"] != "h_iter_phase1_cap0_r1_antecedent_evidence_bound" or manifest["mechanism"] != MECHANISM or manifest["run_identity"] != RUN_ID:
        raise CAP0ContractError("CAP0-R1 antecedent identity differs")
    if manifest["manifest_sha256"] != sha256_bytes(canonical_json({key: value for key, value in manifest.items() if key != "manifest_sha256"})):
        raise CAP0ContractError("CAP0-R1 antecedent self hash differs")


def validate_antecedent(repo: Path) -> dict[str, Any]:
    manifest_path = repo / ANTECEDENT_BINDING["manifest_path"]
    manifest = load_canonical(manifest_path)
    if file_sha256(manifest_path) != ANTECEDENT_BINDING["manifest_file_sha256"]:
        raise InfrastructureInvalid("CAP0-R1 antecedent manifest file differs")
    validate_antecedent_payload(manifest)
    expected_assets = [
        (1, PLAN_ASSET_PATHS[12], "6e20a1a2423c73b47a4a7d52e55f05dad3a2abb7deb1fdeb1b19b32328810dde", 1374, "preflight_sha256", "86f38a7c1154735c32e14f7da9c63b0356e551d932570238e94a1f336267ae30"),
        (2, PLAN_ASSET_PATHS[13], "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855", 0, None, None),
        (3, PLAN_ASSET_PATHS[14], "9a271f2a916b0b6ee6cecb2426f0b3206ef074578be55d9bc94f6f3fe3ab86aa", 2, None, None),
        (4, PLAN_ASSET_PATHS[15], "0d99c5c3cf6f40a31b4ee0f161ea7b73f3bf0eda04831f5a2da400c9a82ab8f1", 12097, "failure_sha256", "7fd9b5516de5454d793efae545f1debaa210294e69501647dad2359b607c24f8"),
        (5, PLAN_ASSET_PATHS[16], "c5dc0c4ea37e83ad48d8cedea4fcc23963f4136864977f5a14b8f921b63c1e25", 561, None, None),
        (6, PLAN_ASSET_PATHS[17], "53c234e5e8472b6ac51c1ae1cab3fe06fad053beb8ebfd8977b010655bfdd3c3", 2, None, None),
    ]
    expected_rows = [{"order": order, "path": path, "file_sha256": digest, "bytes": size, "internal_hash_field": field, "internal_hash_value": internal} for order, path, digest, size, field, internal in expected_assets]
    if manifest["ordered_evidence_assets"] != expected_rows:
        raise CAP0ContractError("CAP0-R1 antecedent asset table differs")
    for _, path, digest, size, field, internal in expected_assets:
        data = read_regular(repo / path)
        if len(data) != size or sha256_bytes(data) != digest:
            raise InfrastructureInvalid(f"CAP0-R1 antecedent asset differs: {path}")
        if field is not None:
            if not data.endswith(b"\n"):
                raise CAP0ContractError("CAP0-R1 antecedent JSON framing differs")
            value = strict_loads(data[:-1])
            if data != canonical_json(value) + b"\n" or value.get(field) != internal:
                raise CAP0ContractError("CAP0-R1 antecedent JSON differs")
    prior = manifest["prior_preflight"]
    prior_keys = {"status", "execution_commit", "mechanism_code_commit", "run_identity", "plan_file_sha256", "plan_sha256", "tree", "receipt_path", "receipt_file_sha256", "receipt_internal_sha256", "stderr_path", "stderr_sha256", "exit_path", "exit_sha256", "cuda_visible_devices", "torch_imported", "tokenizer_loaded", "model_loaded", "scientific_exposure", "output_namespace_absent"}
    if set(prior) != prior_keys or prior["status"] != "h_iter_phase1_cap0_preflight_validated" or prior["execution_commit"] != "e9ca615ad508f96dd405a9aa1ffcaf6f318d78f1" or prior["mechanism_code_commit"] != "9dc3cae7a7a5b11dea81e75c3a4e4e8525cdd965" or prior["plan_file_sha256"] != "c3ec28afff3fc31d2085a8e748545ad68c126239de346acfe280af158d48430b" or prior["plan_sha256"] != "e0fc44b8697f9692217467d21096e315ad98582a674ec19442be43cf592206f4" or prior["tree"] != "f4a4932d6e521ca2249045d1fac6260f8e53e601" or prior["cuda_visible_devices"] != "" or any(prior[key] is not False for key in ("torch_imported", "tokenizer_loaded", "model_loaded", "scientific_exposure")) or prior["output_namespace_absent"] is not True:
        raise CAP0ContractError("CAP0-R1 prior preflight facts differ")
    failure = manifest["antecedent_failure"]
    failure_keys = {"status", "execution_commit", "mechanism_code_commit", "run_identity", "plan_file_sha256", "plan_sha256", "terminal_path", "terminal_file_sha256", "terminal_internal_sha256", "launcher_log_path", "launcher_log_sha256", "exit_path", "exit_sha256", "terminal_error", "root_error", "stage", "model_loaded", "tokenizer_calls_completed", "model_forwards_completed", "sequences_completed", "probes_completed", "cache_checks_completed", "protected_disk_exact", "e33_state_before", "e33_state_after", "e33_state_equal", "e33_grads_present", "model_updated", "validation_opens", "heldout_opens", "h176_loaded", "scientific_exposure", "cap0_scientific_result", "old_output_root"}
    if set(failure) != failure_keys or failure["status"] != "infrastructure_invalid" or failure["terminal_error"] != "CAP0 protected failure audit incomplete" or failure["root_error"] != "CAP0 loaded state tree differs" or failure["stage"] != "model_load" or failure["model_loaded"] is not True or any(failure[key] != 0 for key in ("tokenizer_calls_completed", "model_forwards_completed", "sequences_completed", "probes_completed", "cache_checks_completed", "validation_opens", "heldout_opens")) or failure["protected_disk_exact"] is not True or failure["e33_state_before"] != failure["e33_state_after"] or failure["e33_state_before"] != "a244a54c0a5b6e9a6e91b32e2979dd8afe64450c7fdc67b2af0bc7a3f1098be2" or any(failure[key] is not False for key in ("e33_grads_present", "model_updated", "h176_loaded", "scientific_exposure", "cap0_scientific_result")):
        raise CAP0ContractError("CAP0-R1 antecedent failure facts differ")
    repair = manifest["repair_boundary"]
    expected_repair = {"cause": "model_state_inner_tensor_hash_dialect_mismatch", "model_state_hash_algorithm_before": "sha256(canonical_json(dtype_shape)+LF+raw_contiguous_bytes)", "model_state_hash_algorithm_after": "sha256(raw_contiguous_bytes)", "scientific_tensor_hash_algorithm": "sha256(canonical_json(dtype_shape)+LF+raw_contiguous_bytes)", "scientific_tensor_hash_unchanged": True, "fresh_namespace": True, "old_terminal_reused": False, "prior_scientific_result_reusable": False, "science_changed": False, "thresholds_changed": False}
    if repair != expected_repair:
        raise CAP0ContractError("CAP0-R1 repair boundary differs")
    return ANTECEDENT_BINDING
...
def static_guard(repo: Path) -> dict[str, Any]:
    paths = ["src/prime_rl/latent/h_iter_phase1_cap0_r1.py", "scripts/latent/run_h_iter_phase1_cap0_r1_v1.py"]
...
class ArtifactWriter:
    ...
    def write(self, name: str, payload: dict[str, Any]) -> bytes:
        if self.terminal_written or name not in {"CAP0-R1-PROOF.json", "CAP0-R1-FAILURE.json"} or list(self.output.iterdir()):
...
def tensor_bytes_sha256(torch: Any, tensor: Any) -> str:
    raw = tensor.detach().cpu().contiguous().reshape(-1).view(torch.uint8).numpy().tobytes()
...
def module_tree_sha256(torch: Any, model: Any) -> str:
    ...
    for name, tensor in sorted(model.state_dict().items()):
        digest.update(canonical_json({"name": name, "dtype": str(tensor.dtype), "shape": list(tensor.shape), "sha256": tensor_bytes_sha256(torch, tensor)}))
        digest.update(b"\n")
    return digest.hexdigest()
...
def validate_proof(proof: dict[str, Any], *, plan: dict[str, Any], contract: dict[str, Any], selection: dict[str, Any], execution_commit: str, plan_file_sha256: str) -> None:
    ...
    if set(tampers) != {"results", "rejected_count"} or tampers["rejected_count"] != 55 or [row.get("name") for row in tampers["results"]] != TAMPERS or any(set(row) != {"name", "rejected", "error_type"} or row["rejected"] is not True or row["error_type"] != "CAP0ContractError" for row in tampers["results"]):
...
def validate_preflight(value: dict[str, Any], *, execution_commit: str, plan_file_sha256: str, plan: dict[str, Any]) -> None:
    ...
    if value["execution_commit"] != execution_commit or value["mechanism_code_commit"] != plan["mechanism_code_commit"] or value["plan_file_sha256"] != plan_file_sha256 or value["plan_sha256"] != plan["plan_sha256"] or value["asset_count"] != 25:
    ...
    if not isinstance(static, dict) or set(static) != {"paths", "forbidden_sites", "negative_fixtures", "runner_sha256", "module_sha256"} or static["paths"] != ["src/prime_rl/latent/h_iter_phase1_cap0_r1.py", "scripts/latent/run_h_iter_phase1_cap0_r1_v1.py"] or static["forbidden_sites"] != [] or set(static["negative_fixtures"]) != {"generation_call", "backward_call", "optimizer_step", "candidate_constructor", "validation_path"} or not all(value is True for value in static["negative_fixtures"].values()) or static["runner_sha256"] != plan["asset_sha256"][PLAN_ASSET_PATHS[21]] or static["module_sha256"] != plan["asset_sha256"][PLAN_ASSET_PATHS[20]]:
...
def run_r1_tampers(contract: dict[str, Any], selection: dict[str, Any], capture: dict[str, Any], antecedent: dict[str, Any]) -> list[dict[str, Any]]:
    results: list[dict[str, Any]] = []
    for name in TAMPERS[42:]:
        altered_contract = copy.deepcopy(contract)
        altered_manifest = copy.deepcopy(antecedent)
        target = "manifest"
        if name == "antecedent_manifest_file_hash_changed":
            altered_contract["antecedent_evidence_binding"]["manifest_file_sha256"] = "0" * 64
            target = "contract"
        elif name == "antecedent_manifest_internal_hash_changed":
            altered_contract["antecedent_evidence_binding"]["manifest_internal_sha256"] = "0" * 64
            target = "contract"
        elif name == "antecedent_failure_file_hash_changed": altered_manifest["ordered_evidence_assets"][3]["file_sha256"] = "0" * 64
        elif name == "antecedent_failure_internal_hash_changed": altered_manifest["ordered_evidence_assets"][3]["internal_hash_value"] = "0" * 64
        elif name == "antecedent_launcher_log_hash_changed": altered_manifest["ordered_evidence_assets"][4]["file_sha256"] = "0" * 64
        elif name == "antecedent_exit_hash_changed": altered_manifest["ordered_evidence_assets"][5]["file_sha256"] = "0" * 64
        elif name == "antecedent_preflight_file_hash_changed": altered_manifest["ordered_evidence_assets"][0]["file_sha256"] = "0" * 64
        elif name == "antecedent_preflight_internal_hash_changed": altered_manifest["ordered_evidence_assets"][0]["internal_hash_value"] = "0" * 64
        elif name == "antecedent_preflight_stderr_hash_changed": altered_manifest["ordered_evidence_assets"][1]["file_sha256"] = "0" * 64
        elif name == "antecedent_preflight_exit_hash_changed": altered_manifest["ordered_evidence_assets"][2]["file_sha256"] = "0" * 64
        elif name == "module_state_hash_dialect_changed": altered_manifest["repair_boundary"]["model_state_hash_algorithm_after"] = altered_manifest["repair_boundary"]["model_state_hash_algorithm_before"]
        elif name == "scientific_tensor_hash_dialect_changed": altered_manifest["repair_boundary"]["scientific_tensor_hash_unchanged"] = False
        elif name == "old_namespace_reuse_allowed": altered_manifest["repair_boundary"]["old_terminal_reused"] = True
        else: raise CAP0ContractError(f"CAP0-R1 tamper has no mutation: {name}")
        try:
            if target == "contract":
                altered_contract["contract_sha256"] = sha256_bytes(canonical_json({key: value for key, value in altered_contract.items() if key != "contract_sha256"}))
                validate_contract(altered_contract, selection, capture)
            else:
                altered_manifest["manifest_sha256"] = sha256_bytes(canonical_json({key: value for key, value in altered_manifest.items() if key != "manifest_sha256"}))
                validate_antecedent_payload(altered_manifest)
        except CAP0ContractError:
            results.append({"name": name, "rejected": True, "error_type": "CAP0ContractError"})
        else:
            raise CAP0ContractError(f"CAP0-R1 antecedent tamper accepted: {name}")
    return results
...
def run_full(args: argparse.Namespace) -> None:
    ...
    try:
        with network:
            ...
            antecedent_manifest = load_canonical(repo / ANTECEDENT_BINDING["manifest_path"])
            r1_tamper_results = run_r1_tampers(contract, tamper_selection, tamper_capture, antecedent_manifest)
            ...
            writer.write("CAP0-R1-PROOF.json", proof)
            signal.alarm(0)
        ...
        writer.write("CAP0-R1-FAILURE.json", failure)
        signal.alarm(0)
...
def validate_published(args: argparse.Namespace) -> None:
    ...
    if names == ["CAP0-R1-PROOF.json"]:
        ...
        validate_proof(proof, plan=plan, contract=contract, selection=selection, execution_commit=args.execution_commit, plan_file_sha256=args.plan_file_sha256)
    elif names == ["CAP0-R1-FAILURE.json"]:
        data = read_regular(output / names[0])
        if not data.endswith(b"\n"):
