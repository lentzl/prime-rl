...
if [[ $# -ne 3 || ! $1 =~ ^[0-9a-f]{40}$ || ! $2 =~ ^[0-9a-f]{64}$ || $3 != a1-nc0-cap768-redesign-run1 ]]; then
  echo "usage: $0 <exact-execution-commit> <authorized-plan-sha256> <fresh-run-id>" >&2
  exit 64
fi
readonly execution_commit=$1 authorized_plan_sha256=$2 run_id=$3
...
readonly plan="$experiment/a1-nc0-cap768-redesign-plan-v1.json"
...
readonly output_root=/home/ubuntu/rlm/outputs/latent-a1-nc0-cap768-redesign-v1
...
timeout --signal=TERM --kill-after=60s 3600s "$uv_bin" run --project "$shared_project" --no-sync python \
  "$repo/scripts/latent/run_a1_nc0_cap768_redesign_v1.py" --repo "$repo" --plan "$plan" \
