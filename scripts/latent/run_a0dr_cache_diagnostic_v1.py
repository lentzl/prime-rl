...
OUTPUT_ROOT = Path("/home/ubuntu/rlm/outputs/latent-a0dr-cache-diagnostic-v1")
...
class ArtifactWriter:
    def __init__(self, output_dir: Path) -> None:
        ...
        if not output_dir.name.startswith("a0dr-") or "/" in output_dir.name:
...
def build_representations(model, tokenizer, example: dict[str, object]) -> tuple[dict[str, object], torch.Tensor]:
    ...
    fixed_ids = torch.tensor([[40, 4021, 2528, 8976, 35139, 635, 524, 599]], dtype=torch.long, device="cuda:0")
    if (
        injection_index != 40
        or tensor_sha256(fixed_ids) != "e86e01e61315008783cc217a5bb83a1b3aced0daaecbc920b8d3b45ab4b205d8"
        or any(token_id in set(tokenizer.all_special_ids) for token_id in fixed_ids.flatten().tolist())
        or bool((fixed_ids < 0).any())
        or bool((fixed_ids >= model.config.text_config.vocab_size).any())
    ):
        raise DiagnosticIncomplete("length-control token IDs, boundary, or ordinary-token status changed")
    length_ids = torch.cat((child_ids[:, :injection_index], fixed_ids, child_ids[:, injection_index:]), dim=1)
    ...
    if (
        soft.workspace_span != (injection_index, injection_index + 8)
        or length_ids.shape[1] != 55
        or length_embeddings.shape[1] != 55
        or soft.inputs_embeds.shape[1] != 55
        or not torch.equal(length_mask, soft.attention_mask)
        or not torch.equal(length_positions, soft.position_ids)
    ):
        raise DiagnosticIncomplete("length-matched discrete control or soft-slot alignment changed")
    representations: dict[str, object] = {
        "D47": {"input_ids": child_ids, "attention_mask": mask, "position_ids": None},
        "E47": {"inputs_embeds": exact_embeddings, "attention_mask": mask, "position_ids": positions},
        "L_ID55": {"input_ids": length_ids, "attention_mask": length_mask, "position_ids": length_positions},
        "L_E55": {
        ...
        "fixture": {
            ...
            "injection_index": injection_index,
            "length_control_token_ids": fixed_ids.flatten().tolist(),
            "length_control_token_ids_sha256": tensor_sha256(fixed_ids),
            "length_control_tokens_non_special": True,
            "length_control_input_ids_sha256": tensor_sha256(length_ids),
...
def run(
    ...
    arms = []
    for arm_name in ("D47", "E47", "L_ID55", "L_E55", "S55"):
...
def failure_record(
    ...
    failure: dict[str, object] = {
        "schema_version": "prime-rl/latent-a0dr-cache-diagnostic-failure/v1",
...
def main() -> None:
    ...
    signal.alarm(28 * 60)
    try:
        plan, bank = load_and_validate_a0dr_plan(args.plan, args.bank)
