...
if [[ "${A0CAL_OWNER_APPROVED:-}" != "1" ]]; then
  echo "A0CAL_OWNER_APPROVED=1 is required after root and evaluator review" >&2
  exit 2
fi

repo=${1:?absolute repository path required}
run_id=${2:?exclusive A0 cache-calibration run ID required}
plan="$repo/experiments/qwen35-2b-latent-workspace-v1/a0-cache-calibration-plan-v1.json"
...
output_root=/home/ubuntu/rlm/outputs/latent-a0-cache-calibration-v1
...
if [[ ! "$run_id" =~ ^a0dr-cal-[a-z0-9][a-z0-9._-]{2,59}$ ]]; then
  echo "run ID must use the fresh a0dr-cal- namespace" >&2
  exit 2
fi
if [[ -L "$plan" || -L "$bank" || ! -f "$plan" || ! -f "$bank" ]]; then
  echo "frozen A0 cache-calibration plan or bank is absent or symlinked" >&2
  exit 2
fi
...
if [[ -L "$output_root" ]]; then
  echo "A0 cache-calibration output root must not be a symlink" >&2
  exit 2
fi
install -d -m 700 "$output_root"
if [[ -e "$output_dir" || -L "$output_dir" ]]; then
  echo "refusing to reuse A0 cache-calibration namespace: $output_dir" >&2
  exit 2
fi
...
if (( available_kib < 8388608 || host_memory_kib < 67108864 )); then
  echo "A0 cache-calibration host below frozen bounds" >&2
  exit 2
fi
if [[ "$(nvidia-smi --query-gpu=name --format=csv,noheader -i 0)" != "NVIDIA RTX A6000" ]]; then
  echo "A0 cache calibration requires the frozen NVIDIA RTX A6000" >&2
  exit 2
fi
...
if (( gpu_memory_used_mib > 512 )) || [[ -n "$compute_pids" ]]; then
  echo "A0 cache-calibration cuda:0 is not idle" >&2
  exit 2
fi
if pgrep -af '[v]llm|[p]rime_rl.*(trainer|orchestrator|inference)' >/dev/null; then
  echo "A0 cache calibration refuses active model services or PrimeRL runtimes" >&2
  exit 2
fi
cd "$repo"
execution_commit=$(git rev-parse HEAD)
if [[ ! "$execution_commit" =~ ^[0-9a-f]{40}$ ]] || [[ -n "$(git status --porcelain --untracked-files=all)" ]]; then
  echo "A0 cache calibration requires an exact clean execution commit" >&2
  exit 2
fi
...
timeout 30m "$uv_bin" run --project "$shared_project" --no-sync python \
  "$repo/scripts/latent/run_a0_cache_calibration_v1.py" \
  --repo "$repo" --plan "$plan" --bank "$bank" \
...
if [[ "$output_entries" != "receipt.json" || "$output_bytes" -gt 16777216 ]]; then
  echo "A0 cache-calibration output invalid or exceeds 16 MiB" >&2
  exit 2
fi
