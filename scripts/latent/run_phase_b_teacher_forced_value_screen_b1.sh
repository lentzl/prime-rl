...
readonly OUTPUT_DIR=/home/ubuntu/rlm/results/q35-2b-phase-b1-teacher-forced-value-screen-v1
...
[[ $(sha256sum "$EXPERIMENT_DIR/phase-b-teacher-forced-value-screen-b1-plan.json" | cut -d' ' -f1) == "$ROOT_AUTHORIZED_PLAN_SHA256" ]]
(cd "$EXPERIMENT_DIR" && sha256sum --check phase-b-teacher-forced-value-screen-b1.sha256)
...
exec timeout --signal=TERM --kill-after=60s 4h \
  ...
  --plan "$EXPERIMENT_DIR/phase-b-teacher-forced-value-screen-b1-plan.json" \
