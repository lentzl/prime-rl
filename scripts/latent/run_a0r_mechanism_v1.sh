...
if [[ "${A0R_OWNER_APPROVED:-}" != "1" ]]; then
  echo "A0R_OWNER_APPROVED=1 is required after root reviews the frozen repair plan" >&2
  exit 2
fi

repo=${1:?absolute repository path required}
run_id=${2:?exclusive A0R run ID required}
plan="$repo/experiments/qwen35-2b-latent-workspace-v1/a0r-mechanism-plan-v1.json"
...
output_root=/home/ubuntu/rlm/outputs/latent-a0r-mechanism-v1
...
if [[ "$repo" != /* || -L "$repo" || ! -d "$repo" ]]; then
  echo "repository must be an absolute, existing, non-symlink directory" >&2
  exit 2
fi
if [[ ! "$run_id" =~ ^a0r-[a-z0-9][a-z0-9._-]{2,63}$ ]]; then
  echo "run ID must be a0r- followed by 3-64 lowercase namespace characters" >&2
  exit 2
fi
if [[ -L "$plan" || -L "$bank" || ! -f "$plan" || ! -f "$bank" ]]; then
  echo "frozen A0R plan or bank is absent or symlinked" >&2
  exit 2
fi
...
if [[ ! -x "$uv_bin" || ! -d "$shared_venv" ]]; then
  echo "frozen shared uv executable or project environment is absent" >&2
  exit 2
fi
if [[ -L "$output_root" ]]; then
  echo "A0R output root must not be a symlink" >&2
  exit 2
fi
install -d -m 700 "$output_root"
if [[ -e "$output_dir" || -L "$output_dir" ]]; then
  echo "refusing to reuse A0R output namespace: $output_dir" >&2
  exit 2
fi

available_kib=$(df -Pk "$output_root" | awk 'NR==2 {print $4}')
if (( available_kib < 8388608 )); then
  echo "A0R requires at least 8 GiB free disk" >&2
  exit 2
fi
host_memory_kib=$(awk '/^MemTotal:/ {print $2}' /proc/meminfo)
if (( host_memory_kib < 67108864 )); then
  echo "A0R requires at least 64 GiB host RAM" >&2
  exit 2
fi
if [[ "$(nvidia-smi --query-gpu=name --format=csv,noheader -i 0)" != "NVIDIA RTX A6000" ]]; then
  echo "A0R cuda:0 is not the frozen NVIDIA RTX A6000 resource" >&2
  exit 2
fi
gpu_memory_used_mib=$(nvidia-smi --query-gpu=memory.used --format=csv,noheader,nounits -i 0)
if (( gpu_memory_used_mib > 512 )); then
  echo "A0R cuda:0 is not idle: ${gpu_memory_used_mib} MiB already allocated" >&2
  exit 2
fi
compute_pids=$(nvidia-smi --query-compute-apps=pid --format=csv,noheader,nounits -i 0 2>/dev/null || true)
if [[ -n "$compute_pids" ]]; then
  echo "A0R cuda:0 has active compute processes: $compute_pids" >&2
  exit 2
fi
if pgrep -af '[v]llm|[p]rime_rl.*(trainer|orchestrator|inference)' >/dev/null; then
  echo "A0R refuses to launch while a model service or PrimeRL runtime is active" >&2
  exit 2
fi

cd "$repo"
execution_commit=$(git rev-parse HEAD)
if [[ ! "$execution_commit" =~ ^[0-9a-f]{40}$ ]]; then
  echo "A0R execution commit is not an exact lowercase 40-character hash" >&2
  exit 2
fi
if [[ -n "$(git status --porcelain --untracked-files=all)" ]]; then
  echo "A0R refuses to launch from a dirty worktree" >&2
  exit 2
fi
...
timeout 30m "$uv_bin" run --project "$shared_project" --no-sync python \
  "$repo/scripts/latent/run_a0_mechanism_v1.py" \
  --campaign a0r \
  --repo "$repo" \
  --plan "$plan" \
  --bank "$bank" \
...
if [[ "$output_entries" != "receipt.json" || "$output_bytes" -gt 16777216 ]]; then
  echo "A0R output namespace contains unexpected files or exceeds 16 MiB" >&2
  exit 2
fi
