...
readonly OUTPUT_DIR=/home/ubuntu/rlm/results/q35-2b-phase-b-fixed-depth-smoke-a0c-br2
...
[[ $(sha256sum "$EXPERIMENT_DIR/phase-b-fixed-depth-smoke-a0c-br2-plan.json" | cut -d' ' -f1) == "$ROOT_AUTHORIZED_PLAN_SHA256" ]]
(cd "$EXPERIMENT_DIR" && sha256sum --check phase-b-fixed-depth-smoke-a0c-br2.sha256)
...
exec timeout --signal=TERM --kill-after=30s 120m \
  ...
  --plan "$EXPERIMENT_DIR/phase-b-fixed-depth-smoke-a0c-br2-plan.json" \
