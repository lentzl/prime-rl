...
if [[ "${A0D_OWNER_APPROVED:-}" != "1" ]]; then
  echo "A0D_OWNER_APPROVED=1 is required after root and evaluator review" >&2
  exit 2
fi

repo=${1:?absolute repository path required}
run_id=${2:?exclusive A0D run ID required}
plan="$repo/experiments/qwen35-2b-latent-workspace-v1/a0d-cache-diagnostic-plan-v1.json"
...
output_root=/home/ubuntu/rlm/outputs/latent-a0d-cache-diagnostic-v1
...
if [[ ! "$run_id" =~ ^a0d-[a-z0-9][a-z0-9._-]{2,63}$ ]]; then
  echo "run ID must use a fresh a0d- namespace" >&2
  exit 2
fi
if [[ -L "$plan" || -L "$bank" || ! -f "$plan" || ! -f "$bank" ]]; then
  echo "frozen A0D plan or bank is absent or symlinked" >&2
  exit 2
fi
...
if [[ -L "$output_root" ]]; then
  echo "A0D output root must not be a symlink" >&2
  exit 2
fi
install -d -m 700 "$output_root"
if [[ -e "$output_dir" || -L "$output_dir" ]]; then
  echo "refusing to reuse A0D output namespace: $output_dir" >&2
  exit 2
fi
...
if (( available_kib < 8388608 || host_memory_kib < 67108864 )); then
  echo "A0D host is below the frozen disk or RAM bound" >&2
  exit 2
fi
if [[ "$(nvidia-smi --query-gpu=name --format=csv,noheader -i 0)" != "NVIDIA RTX A6000" ]]; then
  echo "A0D cuda:0 is not the frozen NVIDIA RTX A6000" >&2
  exit 2
fi
...
if (( gpu_memory_used_mib > 512 )) || [[ -n "$compute_pids" ]]; then
  echo "A0D cuda:0 is not idle" >&2
  exit 2
fi
if pgrep -af '[v]llm|[p]rime_rl.*(trainer|orchestrator|inference)' >/dev/null; then
  echo "A0D refuses to launch while a model service or PrimeRL runtime is active" >&2
  exit 2
fi
...
if [[ ! "$execution_commit" =~ ^[0-9a-f]{40}$ ]] || [[ -n "$(git status --porcelain --untracked-files=all)" ]]; then
  echo "A0D requires an exact commit and clean worktree" >&2
  exit 2
fi
...
timeout 30m "$uv_bin" run --project "$shared_project" --no-sync python \
  "$repo/scripts/latent/run_a0d_cache_diagnostic_v1.py" \
  --repo "$repo" \
  --plan "$plan" \
  --bank "$bank" \
...
if [[ "$output_entries" != "receipt.json" || "$output_bytes" -gt 16777216 ]]; then
  echo "A0D output namespace contains unexpected files or exceeds 16 MiB" >&2
  exit 2
fi
