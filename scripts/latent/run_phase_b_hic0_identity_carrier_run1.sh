...
readonly OUTPUT_DIR=/home/ubuntu/rlm/results/q35-2b-b-hic0-identity-carrier-run1
...
[[ $(sha256sum "$EXPERIMENT_DIR/phase-b-hic0-identity-carrier-run1-plan.json" | cut -d' ' -f1) == "$ROOT_AUTHORIZED_PLAN_SHA256" ]]
(cd "$EXPERIMENT_DIR" && sha256sum --check phase-b-hic0-identity-carrier-run1.sha256)
...
exec timeout --signal=TERM --kill-after=60s 4h \
  ...
  --plan "$EXPERIMENT_DIR/phase-b-hic0-identity-carrier-run1-plan.json" \
