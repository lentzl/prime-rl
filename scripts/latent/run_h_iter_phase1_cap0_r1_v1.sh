...
readonly plan_rel=experiments/qwen35-2b-latent-workspace-v1/h-iter-phase1-cap0-r1-v1/cap0-r1-plan.json
readonly sidecar_rel=experiments/qwen35-2b-latent-workspace-v1/h-iter-phase1-cap0-r1-v1/cap0-r1-plan.sha256
readonly output=/home/ubuntu/rlm/outputs/q35-2b-h-iter-phase1-cap0-r1-capture-run1

if [[ ! $execution_commit =~ ^[0-9a-f]{40}$ || ! $plan_sha =~ ^[0-9a-f]{64}$ || $run_id != h-iter-phase1-cap0-r1-capture-run1 ]]; then
...
if [[ $mode == "--preflight-only" && $# -eq 4 ]]; then
  ...
  exec "$python_bin" "$repo/scripts/latent/run_h_iter_phase1_cap0_r1_v1.py" \
...
set +e
"$python_bin" "$repo/scripts/latent/run_h_iter_phase1_cap0_r1_v1.py" \
  --repo "$repo" --execution-commit "$execution_commit" --plan-file-sha256 "$plan_sha" \
...
if [[ $proof_exit -eq 0 ]]; then
  [[ -f $output/CAP0-R1-PROOF.json && ! -e $output/CAP0-R1-FAILURE.json ]]
  exit 0
fi
[[ -f $output/CAP0-R1-FAILURE.json && ! -e $output/CAP0-R1-PROOF.json ]]
exit "$proof_exit"
