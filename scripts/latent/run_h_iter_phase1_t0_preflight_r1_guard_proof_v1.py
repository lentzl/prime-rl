...
def validate_proof(value:dict[str,Any])->None:
    ...
    if not isinstance(rows,list) or len(rows)!=5 or [r.get("name") for r in rows]!=GUARD_CASE_NAMES or [r.get("index") for r in rows]!=list(range(5)) or not all(r.get("qualifies") is True for r in rows): raise RuntimeError("T0 guard proof cases differ")
    ...
    if value["source_evidence"].get("normalized_ast_equal") is not True or value["source_evidence"].get("only_allowed_surface_changed") is not True or value["asset_audit"].get("all_exact") is not True or value["full_freeze"].get("complete") is not True: raise RuntimeError("T0 guard proof closure differs")
...
def run(repo:Path,execution_commit:str,plan_file_sha256:str,output:Path)->None:
    ...
    plan=strict_loads(plan_path.read_bytes()); validate_guard_plan(plan,repo=repo); mark("RUNTIME_VERIFIED")
    ...
    proof["proof_sha256"]=sha(canonical_json({k:v for k,v in proof.items() if k!="proof_sha256"})); validate_proof(proof); STATE["stage"]="terminal_publication"; atomic_terminal(output,PROOF_NAME,proof,validate_proof)
...
def main()->None:
    ...
    if args.validate_terminal:
        ...
        if names==[PROOF_NAME]: validate_proof(strict_loads((output/PROOF_NAME).read_bytes()))
