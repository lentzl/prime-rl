...
if [[ "${NATURAL_YIELD_SDPO_AUDIT_DRY_RUN:-false}" == true ]]; then
  dry_run_root=$(mktemp -d /tmp/natural-yield-sdpo-dry-run.XXXXXX)
  ...
  rl @ "$config" \
    ...
    --run.dir preflight \
    --dry-run
  echo "natural-yield zero-LR SDPO preflight passed"
  exit 0
fi
