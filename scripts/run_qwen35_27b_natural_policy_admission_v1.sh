...
template=${NATURAL_POLICY_ADMISSION_CONFIG:-$experiment/natural-policy-admission.toml}
rung=${NATURAL_POLICY_RUNG:-natural_n1}
model=${1:-Qwen/Qwen3.5-27B}
label=${2:-r7-$rung-admission-r1}
evaluation_root=${PROCEDURAL_HARNESS_OUTPUT_ROOT:-/ephemeral/evals/qwen35-27b-natural-policy-ramp-v1}
start_index=${NATURAL_POLICY_ADMISSION_START_INDEX:-3100000}
...
if [[ ! -f "$template" ]]; then
  echo "natural-policy admission config does not exist: $template" >&2
  exit 1
fi
if [[ ! "$start_index" =~ ^[0-9]+$ ]]; then
  echo "natural-policy admission start index must be non-negative: $start_index" >&2
  exit 1
fi
...
for pattern, replacement, description in (
    ...
    if count != 1:
        raise SystemExit(f"admission config must contain one {description}")
...
print(classify_natural_curriculum_admission(report, sys.argv[2]))
PY
