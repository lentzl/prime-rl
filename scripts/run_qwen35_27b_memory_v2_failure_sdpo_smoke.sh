...
config=${MEMORY_FAILURE_SDPO_CONFIG:-$root/configs/debug/subagent-communication/344-qwen35-27b-memory-v2-failure-sdpo-smoke.toml}
...
if [[ ! -f "$config" ]]; then
  echo "failure-routed SDPO config does not exist: $config" >&2
  exit 1
fi
