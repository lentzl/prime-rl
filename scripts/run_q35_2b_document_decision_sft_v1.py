...
def _validated_dataset(path: Path) -> dict[str, Any]:
    ...
    if schema_version == DIRECT_SUMMARY_SCHEMA:
        from export_q35_2b_document_summary_direct_sft_v1 import NATIVE_REVISION_FAMILIES
        ...
        if native_children:
            expected_families.add("native_child")
        if native_repairs:
        ...
        family_counts_valid = (
            ...
            and (not native_repairs or (
                ...
                and len({c.get("base_slug") for c in native_repairs}) == len(native_repairs)
                and all(c.get("summary_word_count", 0) <= c.get("word_budget", 0) < len(c["summary"])
                        and c.get("masked_message_indices") == [6] for c in native_repairs)
            ))
            and (not native_revisions or (
                bool(native_children) and manifest.get("incorrect_native_draft_masked") is True
                and manifest.get("native_revision_provenance") == "authored_self_review_before_receipt_not_native_gate_feedback"
                and len({(c["family"], c.get("base_slug")) for c in native_revisions}) == len(native_revisions)
                and all(c.get("masked_message_indices") == [4]
                        and c.get("revision_provenance") == manifest["native_revision_provenance"]
                        and isinstance(c.get("incorrect_draft"), str) and c["incorrect_draft"] != c["summary"]
                        and c.get("correction_reasoning", "").strip() for c in native_revisions)
            ))
            and (not native_interruptions or (
                bool(native_children) and manifest.get("incorrect_native_interruption_masked") is True
                and manifest.get("native_interruption_provenance") == "authored_TRAIN_numeric_message_and_empty_call_recovery"
                and manifest.get("native_interruption_parent_message") == "simplified_parent_message_without_native_envelope_ids"
                and manifest.get("native_interruption_feedback") == "current_task_empty_ipython_recovery_not_a_completion_assertion"
                and len(native_interruptions) == len(base_cases)
                and {c.get("base_slug") for c in native_interruptions} == set(base_cases)
                and all(c.get("masked_message_indices") == [2, 5]
                        and c.get("interruption_provenance") == manifest["native_interruption_provenance"]
                        and c.get("parent_message_observation") == manifest["native_interruption_parent_message"]
                        and c.get("parent_message", "").startswith("[from parent]\n\n")
...
def _validated_direct_summary_audit(path: Path, tokenizer_path: Path) -> dict[str, Any]:
    from export_q35_2b_document_summary_direct_sft_v1 import NATIVE_FAMILIES, NATIVE_REVISION_FAMILIES
    ...
    if any(c["family"] in {"format_repair", "semantic_repair", "native_count_repair", "native_interruption_repair"} | NATIVE_REVISION_FAMILIES and (
    ...
    if any(c["family"] in NATIVE_FAMILIES and (
            by_slug[c["slug"]].get("native_child") is not True
            or by_slug[c["slug"]].get("receipt_send_stub_only") is not True
            or by_slug[c["slug"]].get("native_child_execution_verified") is not False
    ) for c in cases):
        raise ValueError("invalid native-child receipt replay audit")
    if any(c["family"] in NATIVE_REVISION_FAMILIES
           and by_slug[c["slug"]].get("native_revision_reasoning_supervised") is not True for c in cases):
        raise ValueError("invalid native-child correction reasoning audit")
...
def _validated_owner_summary_audit(path: Path, source_model: Path) -> dict[str, Any]:
    ...
    if (audit.get("schema_version") != "qwen35-2b-document-summary-owner-renderer-audit/v1"
            ...
            or any(row.get("truncated") is not False or not 0 < row.get("tokens", 0) <= 16384
                   ...
                   or (row.get("family") in {"owner_schema_receipt_repair", "owner_wait_repair", "owner_start_repair", "owner_handle_repair", "owner_message_repair"}
