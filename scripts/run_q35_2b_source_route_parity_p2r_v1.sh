...
config_sha=${3:?root-frozen P2R config SHA-256 required}
task_bank_sha=${4:?root-frozen P2R task-bank SHA-256 required}
task_key_set_sha=${5:?root-frozen P2R task-key-set SHA-256 required}
sampling_sha=${6:?root-frozen P2R sampling-contract SHA-256 required}
failed_start_sha=${7:?root-frozen failed-P2 evidence SHA-256 required}
dry_run=${P2R_DRY_RUN:-false}

config=experiments/qwen35-2b-document-recursion-zero-update-v1/\
specialist-source-route-parity-p2r-lr0.toml
validator=scripts/validate_q35_2b_source_route_parity_p2r_v1.py
sampling_contract=experiments/qwen35-2b-document-recursion-zero-update-v1/\
specialist-source-route-parity-p2r-sampling-v1.json
failed_start_evidence=experiments/qwen35-2b-document-recursion-zero-update-v1/\
...
output_root=/home/ubuntu/rlm/outputs/q35-2b-source-route-parity-p2r-v1
run_dir=$output_root/lr0-calibration/source-route-parity-p2r-lr0-admission
result_root=/home/ubuntu/rlm/results/q35-2b-source-route-parity-p2r-v1
...
preflight_archive=/home/ubuntu/rlm/preflight/q35-2b-source-route-parity-p2r-v1-dryrun-$revision
...
p2r_pythonpath="$root/deps/verifiers:$root/src:$root/packages/prime-rl-configs/src:$root/deps/verifiers/environments/subagent_communication_v1:$root/deps/verifiers/environments/source_worker_first_call_v1"
export PYTHONPATH="$p2r_pythonpath${PYTHONPATH:+:$PYTHONPATH}"

cd "$root"
if [[ "$(git rev-parse HEAD)" != "$revision" ]]; then
  echo "P2R execution revision mismatch" >&2
  exit 1
fi
if [[ -n "$(git status --porcelain)" ]]; then
  echo "P2R requires a clean parent worktree" >&2
  exit 1
fi
if [[ "$(git -C deps/verifiers rev-parse --show-toplevel 2>/dev/null)" != "$root/deps/verifiers" ]]; then
  echo "P2R requires an initialized Verifiers submodule" >&2
  exit 1
fi
...
for package in subagent_communication_v1 source_worker_first_call_v1; do
  if [[ ! -f "$root/deps/verifiers/environments/$package/pyproject.toml" ]]; then
    echo "P2R Verifiers package is missing: $package" >&2
    exit 1
  fi
done
if [[ ! "$revision" =~ ^[0-9a-f]{40}$ \
      ...
      || ! "$sampling_sha" =~ ^[0-9a-f]{64}$ \
      || ! "$failed_start_sha" =~ ^[0-9a-f]{64}$ ]]; then
  echo "P2R requires exact revision and config identities" >&2
  exit 1
fi
test "$(sha256sum "$config" | awk '{print $1}')" = "$config_sha"
test "$(sha256sum "$sampling_contract" | awk '{print $1}')" = "$sampling_sha"
test "$(sha256sum "$failed_start_evidence" | awk '{print $1}')" = "$failed_start_sha"
...
if [[ -e "$output_root" || -e "$result_root" ]]; then
  echo "refusing duplicate or partial P2R output/result root" >&2
  exit 1
fi
...
run_p2r_python "$validator" "$config"
run_p2r_python "$validator" "$config" --verify-core-import-provenance
run_p2r_python "$validator" "$config" --validate-failed-start-evidence \
  --failed-start-evidence-sha256 "$failed_start_sha"
run_p2r_python scripts/hash_q35_2b_source_route_parity_p2_tasks_v1.py "$config" \
  --expected-task-bank-sha256 "$task_bank_sha" \
  --expected-task-key-set-sha256 "$task_key_set_sha"

if [[ "$dry_run" == true ]]; then
  if [[ -e "$preflight_archive" ]]; then
    echo "refusing duplicate P2R dry-run archive" >&2
    exit 1
  fi
  run_p2r_rl "$config" --dry-run
  if [[ ! -d "$run_dir/configs" ]]; then
    echo "P2R dry run did not emit resolved configs" >&2
    exit 1
  fi
  ...
  archived_run=$preflight_archive/output-root/lr0-calibration/source-route-parity-p2r-lr0-admission
  run_p2r_python "$validator" "$archived_run" --validate-resolved-configs
  if [[ -e "$output_root" || -e "$result_root" ]]; then
    echo "P2R dry run left actual output/result namespaces occupied" >&2
    exit 1
  fi
  echo "P2R static and launcher dry-run checks passed"
  exit 0
fi

mapfile -t gpu_names < <(nvidia-smi --query-gpu=name --format=csv,noheader)
if [[ ${#gpu_names[@]} -ne 2 ]] || printf '%s\n' "${gpu_names[@]}" | grep -qv 'RTX A6000'; then
  echo "P2R requires exactly 2x RTX A6000" >&2
  exit 1
fi
if [[ -n "$(nvidia-smi --query-compute-apps=pid --format=csv,noheader)" ]]; then
  echo "refusing P2R launch while another GPU process is active" >&2
  exit 1
fi
...
if [[ ! -f "$run_dir/metrics.jsonl" \
  ...
  echo "P2R did not produce one complete unrecorded calibration" >&2
  exit 1
fi
if find "$run_dir/checkpoints" "$run_dir/weights" -type f -print -quit 2>/dev/null | grep -q .; then
  echo "P2R wrote a forbidden model artifact" >&2
  exit 1
fi
rehash_protected_models
run_p2r_python "$validator" "$run_dir" \
  --write-postflight-model-hashes "$model_postflight" \
  --config-sha256 "$config_sha" \
  --execution-revision "$revision" \
  --verifiers-revision "$verifiers_revision"
test -f "$model_postflight"
run_p2r_python "$validator" "$run_dir" --runtime \
  --config-sha256 "$config_sha" \
  ...
  --core-import-provenance "$core_import_provenance" \
  --output "$admission_result"
rehash_protected_models
test -f "$admission_result"
echo "P2R mechanism admission recorded; no checkpoint or nonzero optimizer authorization: $admission_result"
